...
async function main() {
  ...
  const criticalPackages = {
    ...
    "express-validator":
      currentPackage.dependencies?.["express-validator"] || "Not found",
  };
  ...
  if (auditResult) {
    try {
      ...
      if (summary?.data?.vulnerabilities) {
        ...
        log(
          `   Found: ${vulns.critical || 0} critical, ${vulns.high || 0} high, ${vulns.moderate || 0} moderate`,
  ...
  if (breakingChanges.length > 0) {
    ...
    breakingChanges.forEach((change) => {
      ...
      log(
        `   - ${change.type}: ${change.files} file(s) - ${change.action}`,
        severityColor,
      );
    });
  } else {
  ...
  if (!isDryRun) {
    ...
    log(
      "\n📥 Step 5: Installing updated dependencies... (skipped in dry-run)",
      "yellow",
    );
  }
  ...
  if (!isDryRun) {
    ...
    if (typeCheckResult) {
      ...
      log(
        "   ⚠️  Type errors found - review and fix before proceeding",
        "yellow",
      );
    }
  } else {
    log(
      "\n🔎 Step 6: Running TypeScript type check... (skipped in dry-run)",
      "yellow",
    );
  }
  ...
  if (breakingChanges.length > 0) {
    log(
      `\n⚠️  ${breakingChanges.length} potential breaking change(s) detected`,
      "yellow",
    );
